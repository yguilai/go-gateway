...
import (
	...
	"github.com/yguilai/go-gateway/reverse_proxy/load_balance"
	"strings"
	"sync"
)
...
func (t *LoadBalance) Find(c *gin.Context, tx *gorm.DB, rule *LoadBalance) (*LoadBalance, error) {
	m := &LoadBalance{}
	err := tx.SetCtx(public.GetGinTraceContext(c)).Where(rule).Find(m).Error
	return m, err
}
