...
func (t *AccessControl) Find(c *gin.Context, tx *gorm.DB, rule *AccessControl) (*AccessControl, error) {
	model := &AccessControl{}
	err := tx.SetCtx(public.GetGinTraceContext(c)).Where(rule).Find(model).Error
	return model, err
}
